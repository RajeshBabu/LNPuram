	...
	*/
	var TreeNode = function (nodeStructure, id, parentId, tree, stackParentId) {
		...
		this.image = nodeStructure['image'];
                this.wifeimage = nodeStructure['wifeimage'];
			...
			if (!this.nodeInnerHTML) {
				...
				if(this.image) {
					...
					node.appendChild(image);
                                ...
                                // IMAGE
				if(this.wifeimage) {
					wifeimage = document.createElement('img');
					wifeimage.src = this.wifeimage;
					node.appendChild(wifeimage);
				}
	...
	Tree.prototype.CONFIG = {
		...
		'padding': 5, // the difference is seen only when the scrollbar is shown
